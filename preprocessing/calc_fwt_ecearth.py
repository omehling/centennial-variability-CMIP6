...
aux_path = "/data/datasets/synda/data/CMIP6/CMIP/EC-Earth-Consortium/EC-Earth3/piControl/r2i1p1f1/Omon/thetao/gn/v20210601/thetao_Omon_EC-Earth3_piControl_r2i1p1f1_gn_210301-210312.nc"
...
root_path = "/data/datasets/synda/data/CMIP6/CMIP/EC-Earth-Consortium/EC-Earth3/piControl/r2i1p1f1/Omon/"
...
s_mean = xr.open_dataset("/home/omehling/work/cmip6/piControl_salinity/so_mean_ao/soao_Omon_EC-Earth3_piControl_r2i1p1f1_gn.nc")["so"].mean().item()
print("Mean salinity: {:.2f}".format(s_mean))

## Fram strait ##
# Fram strait indices
fram_i = slice(268,279)
fram_j = 272
...
for vert_lat, vert_lon in zip(
        list(vertices_lat.isel(i=fram_i, j=fram_j).values),
        list(vertices_lon.isel(i=fram_i, j=fram_j).values)):
    fram_zonal_lengths.append(cell_length_zonal(vert_lat, vert_lon))
fram_zonal_lengths = xr.DataArray(fram_zonal_lengths, coords={"i": vertices_lat.isel(i=fram_i, j=fram_j).i})
...
# BSO indices
bso_len = 11
svalbard_idx = [282, 270]

bso_i = xr.DataArray([svalbard_idx[0]+i for i in range(bso_len)], dims=['locs'])
bso_j = xr.DataArray([svalbard_idx[1]-i for i in range(bso_len)], dims=['locs'])

# Grid cell weights for BSO
bso_lengths = []

for vert_lat, vert_lon in zip(
        list(vertices_lat.isel(i=bso_i, j=bso_j).values),
        list(vertices_lon.isel(i=bso_i, j=bso_j).values)):
    bso_lengths.append(cell_length_diagonal(vert_lat, vert_lon))
bso_lengths = xr.DataArray(bso_lengths, coords={"locs": vertices_lat.isel(i=bso_i, j=bso_j).locs})
...
for load_file_idx in range(len(filenames_vo)):
    ...
    ## Fram strait
    s_fram = s_ece.isel(i=fram_i, j=fram_j)
    vmid_fram = (vo_ece.isel(i=fram_i, j=fram_j)+vo_ece.isel(i=fram_i, j=fram_j+1))/2

    fram_fwt_monthly = (vmid_fram*((s_ref-s_fram)/s_ref)*fram_cell_weights).sum(["lev","i"])
    
    ## BSO
    s_bso = s_ece.isel(i=bso_i, j=bso_j)
    vo_bso_prov = vo_ece.isel(i=bso_i, j=bso_j)
    uo_bso_prov = uo_ece.isel(i=bso_i, j=bso_j)
    ...
    df_fwts.to_csv("fwt_ecearth/fwt_EC-Earth_{:04d}.csv".format(load_file_idx))
